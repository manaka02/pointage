...
class ImportTableMap extends TableMap
{
     ...
     */
    const OM_CLASS = '\\App\\Models\\Import';
     ...
     */
    const COL_IMPORT_ID = 'import.import_id';

    /**
     * the column name for the target field
     */
    const COL_TARGET = 'import.target';

    /**
     * the column name for the mapping field
     */
    const COL_MAPPING = 'import.mapping';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('ImportId', 'Target', 'Mapping', ),
        self::TYPE_CAMELNAME     => array('importId', 'target', 'mapping', ),
        self::TYPE_COLNAME       => array(ImportTableMap::COL_IMPORT_ID, ImportTableMap::COL_TARGET, ImportTableMap::COL_MAPPING, ),
        self::TYPE_FIELDNAME     => array('import_id', 'target', 'mapping', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('ImportId' => 0, 'Target' => 1, 'Mapping' => 2, ),
        self::TYPE_CAMELNAME     => array('importId' => 0, 'target' => 1, 'mapping' => 2, ),
        self::TYPE_COLNAME       => array(ImportTableMap::COL_IMPORT_ID => 0, ImportTableMap::COL_TARGET => 1, ImportTableMap::COL_MAPPING => 2, ),
        self::TYPE_FIELDNAME     => array('import_id' => 0, 'target' => 1, 'mapping' => 2, ),
        self::TYPE_NUM           => array(0, 1, 2, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\App\\Models\\Import');
        $this->setPackage('');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('import_id', 'ImportId', 'INTEGER', true, null, null);
        $this->addColumn('target', 'Target', 'VARCHAR', false, 50, null);
        $this->addColumn('mapping', 'Mapping', 'VARCHAR', false, null, null);
    } // initialize()
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if ($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ImportId', TableMap::TYPE_PHPNAME, $indexType)] === null) {
            return null;
        }

        return null === $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ImportId', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ImportId', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ImportId', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ImportId', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ImportId', TableMap::TYPE_PHPNAME, $indexType)];
    }
    ...
    public static function getPrimaryKeyFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        return (int) $row[
            $indexType == TableMap::TYPE_NUM
                ? 0 + $offset
                : self::translateFieldName('ImportId', TableMap::TYPE_PHPNAME, $indexType)
        ];
    }
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(ImportTableMap::COL_IMPORT_ID);
            $criteria->addSelectColumn(ImportTableMap::COL_TARGET);
            $criteria->addSelectColumn(ImportTableMap::COL_MAPPING);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \App\Models\Import) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(ImportTableMap::DATABASE_NAME);
            $criteria->add(ImportTableMap::COL_IMPORT_ID, (array) $values, Criteria::IN);
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(ImportTableMap::COL_IMPORT_ID) && $criteria->keyContainsValue(ImportTableMap::COL_IMPORT_ID) ) {
